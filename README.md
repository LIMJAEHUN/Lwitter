...
# 39일차
- 좋아요 진행중
- 쿼리문으로 해당 문서를 찾아 좋아요 취소를 누르면 해당 문서를 삭제 하게 진행
- query로 해당 하는 값을 가지는 객체를 찾은후 그 값을 통해 해당 문서로 접근 후 data에 저장한후 길이를 통하여 0이 아니면 삭제 함수 실행 좋아요 버튼을 누르면 if문 조건에 부합하는 데이터가 1개 이상있으므로 map함수 사용없이 가능
- 좋아요 버튼 누른 값(ex 0 ,1 ) 저장문제 진행할예정 storage 를 통해 누르면값을 storage 에 저장 하여 새로고침 해도 값이 변화가 없도록 진행해볼예정
# 40일차
- 좋아요 진행중
- 좋아요 버튼 누를시 likecount 증가 감소 기능 추가
- 버튼에 좋아요 갯수 표시, 버튼 누르면 바로 감소 증가 반영
- 간단한걸 너무 빙돌아서 왔다......

# 임시 중단
- 기능 적은 부분은 90% 완료
- 다시 시작할때 좋아요 부분(상태 스토리지 저장 ) 부터 시작
- 넷플릭스 진행